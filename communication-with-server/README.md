...
なお、べき等や安全であるなどの性質は仕様として定義されているが、必ずこの性質が守られるわけでは無い。  
この性質が守られるためには、サーバ側でこれを守るように実装されていなければならない。  
## POSTとは
...
GET同様、べき等で無いなどの性質はサーバの実装によるため、HTTPメソッドの仕様通りでは無い場合もある。  
...
ただし、どのHTTPメソッドを利用するにしても、サーバ側の実装はそのメソッドの使用を守って実装する必要がある。  
