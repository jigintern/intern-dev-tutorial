...
> ### TIPS:べき等とは
> クライアント側が何度同じ処理を実行しても、サーバ側が1度しか実行していないときと同じ状態になることを指す。  
> 例えばECサイトの購入ボタンがあり、このボタンを複数回連打しても1回分の購入しか行われない場合、べき等性があると言う。  
> 逆に複数回連打されたとき、複数回購入される場合はべき等性がないと言う。  
> もちろん、購入ボタンの動作としてどちらが正しいかはその仕様書によるが、少なくとも複数回購入が行えるボタンを、GETなどのべき等性があるとされるメソッドで実装してはいけない。  
> べき等性があるメリットとして、ネットワーク障害によってレスポンスが受け取れず再リクエストを行ったとしても、サーバのデータを破壊しないという点がある。
