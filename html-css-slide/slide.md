...
- **文字** … `color` / `font-size` / `font-weight` / `line-height` / `text-decoration` / `font-family`
- **背景・枠・形** … `background`(`-color` / `-image`) / `border` / `border-radius`
- **大きさ・余白** … `width` / `height` / `padding` / `margin` / `box-sizing`
- **表示・その他** … `display` / `cursor`
...
一覧の中の `color` から始めましょう。**1 つのタグに 1 つのスタイル**を当てるのが基本です。

<div class="syntax">

`h1 { color: red; }` … **すべての `<h1>` 要素**の**文字色**を**赤**にする、という意味。
