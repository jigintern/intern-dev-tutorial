...
**1. Repository name — 名前を付ける**

自分のアプリの名前です。ここでは例として`deno-app`としています。

> [!IMPORTANT]
> 名前は**半角の英数字とハイフン(`-`)だけ**で付けてください。
> **スペースとアンダースコア(`_`)は使わないでください。**

この名前は、6 章で公開するときの**URL の一部**になります。あとから変えるのは手間なので、ここで決めてしまいましょう。

- スペースを入れると、GitHub が自動でハイフンに置き換えます。**打った名前と実際の名前がズレる**ので、clone するときに混乱します
- アンダースコア(`_`)は URL に使えないため、**デプロイのときに名前を付け直すことになります**
...
> [!IMPORTANT]
> ここから 6 章のデプロイまで、[3-3](#3-3-自分の-pc-にクローンする)でクローンした**自分のリポジトリ**の中で作業します。
> この教材リポジトリ（`intern-dev-tutorial`）ではないので気をつけてください。

1. VSCode で、クローンしたフォルダ（例: `deno-app`）を開きましょう。
...
デプロイが終わったら、**自分のスマホで URL を開いてみてください。**自分だけの URL で、自分のアプリが表示されます 🎉
