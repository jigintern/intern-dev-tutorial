...
- [1. そもそも API とは](#1-そもそも-api-とは)
  - [1-1. API は「窓口」](#1-1-api-は窓口)
    - [窓口があると、うれしいことが 2 つあります](#窓口があるとうれしいことが-2-つあります)
    - [窓口のルールは、この 4 つ](#窓口のルールはこの-4-つ)
    - [4 つを組み合わせると、コードになります](#4-つを組み合わせるとコードになります)
  - [1-2. `server.js` は、全部の入口](#1-2-serverjs-は全部の入口)
    - [これが昨日の最初の形](#これが昨日の最初の形)
    - [上から順に「自分の担当か」を見ています](#上から順に自分の担当かを見ています)
    - [フロントの構成要素も `server.js` が返しています](#フロントの構成要素も-serverjs-が返しています)
    - [構成要素は 1 つずつ、4 回に分けて返ります](#構成要素は-1-つずつ4-回に分けて返ります)
    - [自分のサイトで見てみよう](#自分のサイトで見てみよう)
    - [見る場所は 2 つあります](#見る場所は-2-つあります)
    - [（参考）大きなサービスでは分かれています](#参考大きなサービスでは分かれています)
    - [みんなの `server.js` は、中身が違います](#みんなの-serverjs-は中身が違います)
    - [なぜ `serveDir` の「上」なのか](#なぜ-servedir-の上なのか)
...
**Application Programming Interface**の略です。名前がそのまま意味になっています。

| 頭文字 | | 意味 |
| --- | --- | --- |
| **A** | pplication | アプリケーションを |
| **P** | rogramming | プログラムから使うための |
| **I** | nterface | **窓口** |

`Interface`は`inter`（間）＋`face`（面）で、**間にある面**という意味です。

銀行や役所の窓口を想像してください。**中で何をしているかは見えません**が、決まった形で頼めば答えが返ってきます。API も同じです。

そして窓口は、**サーバー側の表面**にあります。銀行の窓口が銀行の一部なのと同じで、`server.js`が窓口を持っていて、ブラウザはそれを叩くだけです。

```
ブラウザ  ──決まった形で頼む──→  ┃ server.js
         ←──── 答えが返る ────  ┃
                                 ↑ この面が窓口 ＝ API
```

### 窓口があると、うれしいことが 2 つあります
...
| 地図 | 膨大な画像データから、その地点を切り出す | `fetch("/map")` |
| 翻訳 | 大量の文章で学習したモデルで推論 | `fetch("/translate")` |

左側は**知らなくていい**んです。書くのは右の 1 行だけ。**自分で気象観測をしなくても、天気を出すアプリがつくれます。**

**利点 2: 見せたくないものを隠せる**

ブラウザ側のコード（`index.html`や`api.js`）は、**開発者ツールで誰でも中身を見られます。**でもサーバー側（`server.js`）の中身は見えません。

だからパスワードの照合・API キー・データベースの情報は、**全部サーバー側に置きます。**この見えない側を**バックエンド**と呼びます。

5 章でつくる`/auth`が、まさにこれです。パスワードの照合をサーバー側でやります。

### 窓口のルールは、この 4 つ

窓口をつくるときは、**使い方のルール**を決めます。決めるのは、いつも同じ 4 つです。
...
窓口をつくる場所が`server.js`です。手を動かす前に、**これがどういうファイルなのか**を見ておきます。

### これが昨日の最初の形

`server.js`を開いてみてください。テンプレートから始まったときは、こうなっていました。
...
昨日、中身を書き換えた人も**形はこのまま残っています。**

### 上から順に「自分の担当か」を見ています

`Deno.serve(...)`が、**このサーバーへの通信をすべて受け取る入口**です。受け取ったあとは、**上から順番に条件を見ているだけ**です。

```
通信が来た
   ↓
GETで /welcome-message か？ ── はい → バックエンドの情報 を返す（おわり）
   ↓ いいえ
どれにも当たらなかった
   ↓
serveDir が ブラウザの構成要素 を返す
```

一番下の`serveDir`は、**どの`if`にも当たらなかったとき**の受け皿です。`public`フォルダの中から、パスに合うファイルを探して返してくれます。

### フロントの構成要素も `server.js` が返しています

ここで気づいてほしいことがあります。`server.js`が返しているものは、**2 種類**あります。

```
                    server.js
                   ↙         ↘
    フロントの構成要素        バックエンドの情報
  index.html / styles.css      /welcome-message
  index.js                     の文字
```

画面をつくる部品（HTML・CSS・JavaScript）も、データも、**同じ入口から、同じ形で返っています。**だから 1 ファイルで両方できているわけです。

> [!NOTE]
> **どちらも同じ形**というのはこういう意味です。`serveDir`がやっているのは、こういうことです。
>
> ```js
...
> 返ってくる場所も同じで、前の章で出てきた**レスポンスボディ**に入っています。

### 構成要素は 1 つずつ、4 回に分けて返ります

昨日ブラウザで`localhost:8000`を開いたとき、通信は**1 回ではありませんでした。**

| 叩かれた URL | 上から順に見た結果 | 返ってきたもの |
| --- | --- | --- |
| `/` | どれにも当たらない → `serveDir` | `index.html` |
| `/styles.css` | どれにも当たらない → `serveDir` | CSS |
| `/index.js` | どれにも当たらない → `serveDir` | JavaScript |
| `/welcome-message` | **1 番目の`if`に当たった** | 「jig インターンへようこそ！」の文字 |

3 回は**フロントの構成要素**、1 回は**バックエンドの情報**です。どちらも同じ`server.js`が答えています。

**まとめて 1 回では返ってきません。**こういう順番で起きています。
...
| --- | --- | --- |
| 自分の URL | `Other` | 自分でアドレスバーに入れた |
| `styles.css` | `(index):8` | `index.html`の 8 行目 |
| `index.js` | `(index):9` | `index.html`の 9 行目 |
| `welcome-message` | `index.js:1` | `index.js`の 1 行目 |

**最初の 1 回以外は、前に受け取ったファイルが次を呼んでいます。**さっきの「HTML を受け取ってから順に取りに行く」が、この列に出ています。

`welcome-message`の行をクリックして、**Response**タブ（または「応答」）を開くと、返ってきた文字がそのまま見られます。

> [!TIP]
> この**Network タブ**は、今日ずっと使えます。
> ボタンを押しても何も起きないとき、ここを見れば「そもそもリクエストが飛んでいないのか」「飛んだけどエラーが返ってきたのか」が分かります。
> **Status**が`200`なら成功、`404`なら「そんな窓口はない」という意味です。
>
> `Size`が`(memory cache)`の行は、**サーバーに取りに行かず手元の控えを使った**という意味です。ターミナルにパスが出ないときは、これが理由かもしれません。`Cmd + Shift + R`（強制再読み込み）で毎回取りに行かせられます。
...
これが 1-1 で見た**窓口**です。外から見えるのは窓口でのやりとりだけ。だから利点 2 のとおり、見せたくないものはサーバー側に置けるわけです。
...
かわりに使うのが、[0-1](#0-1-今日さわるファイル)で決めた**目印**です。`return serveDir(`と`</body>`の 2 つの行は、消してしまうとページが表示されなくなるので、**昨日デプロイできた人には必ず残っています。**
...
進め方は [0-0](#0-0-今日の流れ) で見た 4 ステップです。3 章・4 章・5 章もすべて同じ流れです。

**2 番の「URL 直打ちで確認」を必ず挟みます。**ここを確認しておくと、動かないときに**サーバーとブラウザのどちらが原因か**を自分で切り分けられます。
...
開発者ツール（`F12`）の**Console**タブに、赤いエラーが出ていないか確認してください。症状ごとの対処は [2-5](#2-5-ボタンから叩いて確認する) にまとめてあります。
