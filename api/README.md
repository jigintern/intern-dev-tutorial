...
> これまでにつくったページはそのまま残して、今日の分は`api.html`という**別のページ**につくります。
> これまでの成果を壊す心配がありません。
...
| ページが表示されない | デプロイが終わっていないかもしれません。声をかけてください |
| VSCode に`server.js`が見えない | フォルダが違います。クローンしたフォルダを開き直してください |
...
新しいビルドが走り、**緑のチェックが付けば反映済み**です。デプロイしたときに見たのと同じ画面です。
...
まず、今日の画面をつくります。**すでにある`index.html`とは別の、新しいページです。**
...
**もとのページとは、別の URL です**。トップページを開いても、今日のページは出てきません。
...
`public`の中に置いたファイルは、**URL の後ろにファイル名を付ける**と開けます。`api.html`を置いたので、`/api.html`で開ける、というだけの話です。

> [!IMPORTANT]
> **ただし、いま開いてもまだ`Not Found`です。**
> つくったファイルは、まだ自分の PC の中にしかありません（[0-3](#0-3-直したら毎回-push-します)）。
> 実際に開けるようになるのは、[1-5](#1-5-push-して確認する) で push したあとです。

> [!NOTE]
> **なぜ`/api.html`で開けるのでしょうか。**
> `https://自分のURL/api.html`を開くと、`server.js`の`if`はどれにも当たりません。
...
> API だけでなく、**これまでにつくったページも表示されなくなる**ので、いちばん焦るパターンです。
>
...
| 症状 | 対処 |
| --- | --- |
| ボタンも入力欄も出てこない | もとのページを開いています。URL の後ろに`/api.html`が付いているか確認してください |
| ページ自体が`Not Found` | `api.html`が push できていません。ソース管理タブで`+`を押したか確認してください |
| もとのページまで表示されなくなった | `async`の書き忘れです。[1-2](#1-2-serverjs-の-1-行を書き換える)に戻ってください |
...
> パスワードをちゃんと扱う方法は [ログインハンズオン](../did-login/README.md) にまとめてあります。興味がある人は読んでみてください。
