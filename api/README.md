...
窓口には「どう頼めばいいか」の**決めごと**が必要です。今日つくるのはこれです。決めることは、いつも同じ 4 つです。
...
**`/greeting-me`に`name`を渡すと、`Hello, {渡した名前}`が返ってくる**決めごとです。
...
これでも動きますが、受け取った側が`,`で切り分ける必要があります。切り分ける決めごとを別に用意しないといけません。**面倒だし、間違いやすいです。**
...
> [!NOTE]
> `undefined`が出た人は、**いま API の決めごとが食い違っている状態**です。
> サーバーが`name`で返しているのに、ブラウザが`userName`を探している、というようなときにこうなります。
> **片方だけ直しても動きません。**両側で同じ名前を使うのが決めごとなので、必ずセットで直します。
...
**キーを増やしてもいいです。**その場合は`api.js`側も直すのを忘れないでください。決めごとは両側セットです。
