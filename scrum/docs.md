...
スプリントバックログには、プロダクトバックログからタスクを洗い出して登録します。
前スプリントで追加されたプロダクトバックログについても、タスクの洗い出しをしてください。

次にタスクを開発チームで担当を割り振ります。
残りのスプリントをみて、ビジョンを実現するために優先するタスクを考えてください。
...
第1スプリントは以下の指摘があったものとしてください。
第2スプリント以降は各自で思いついた指摘を追加してください。
...
この演習では、以下のような振り返りだったものとし、スクラムマスターがサイコロを振り偶数の目なら業務改善成功したとします。
...
チーム関係なしで全員で書き込めるドキュメントをHackMDで用意して、みんなで書き込みましょう。(5分)
書いた項目には誰が書き込んだかわかるように名前を書きましょう。
反省を次はインターンで活かしてみましょう。
