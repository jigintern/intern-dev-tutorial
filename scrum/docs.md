...
1. **テスト**：
実装したプログラムをテストし、要件通りに動作することを確認する。ユニットテスト、結合テスト、システムテスト、受け入れテストなどが含まれる。
...
この手法は、製品やシステムの最終版を完成させる前に要件や機能を具体化します。
時間とコストの管理が重要であり、過度なスコープ[^2]拡大を防ぐための適切なマネジメントが求められます。
主な工程を以下に示します。
...
1. **改善と改良**：
フィードバックに基づいてプロトタイプを改良する。フィードバック収集と改良を繰り返し、プロトタイプを徐々に最終製品に近づける。

1. **最終プロトタイプの完成**：
ユーザーの要求を満たす最終プロトタイプが出来上がるまで改良を重ね、この最終プロトタイプを基に、正式な製品やシステムの開発に移行します。
...
固定的な計画に従うウォーターフォール型開発とは対照的に、反復的で増分的なアプローチが特徴です。
...
- ### スプリント
    - 1〜4週間の固定期間(インターンでは2~3日)で、プロダクトの成果物を作成するための開発サイクル。
...
インターン向けにアレンジしているので、プロダクトオーナーとスクラムマスターは開発チーム兼任します。
大まかな流れは以下の通りです。
...
このリストはプロダクトオーナーがマークダウンなどで管理してください。
