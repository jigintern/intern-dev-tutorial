...
Manual モードなら勝手にコードを書き換えたり、危険なコマンドを実行したりはしないので、安心して使えます。
...
| モード | 動作 |
| --- | --- |
| **Manual**（編集前に確認） | 編集のたびに承認を求める |
| **Edit automatically**（自動で編集） | 確認せずに編集を進める |
| **Plan**（計画をたてて確認） | コードを調べてから、編集の前に計画を提示する |
| **Auto**（自動承認） | 安全と判断した操作は自動で進め、リスクのある操作だけ確認する |

![モード切替メニュー](./imgs/screen-shots/07_modes.png)

最初のうちは **Manual**、「いきなり実装されると不安」というときは **Plan** がおすすめです。Plan モードではClaudeが先に実装計画を提示し、ユーザーが承認して初めて実装（編集・コマンド実行）が始まります。計画を修正したいときは、そのままチャットで依頼できます。
...
- **不安なら Plan モード**: 先に計画を確認してから実装させる
...
- 操作の許可を出しすぎた／不安 → `Manual` や `Plan` に戻す。判断に迷ったらメンターに相談する
