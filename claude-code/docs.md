...
人間が「何をしたいか」を伝え、Claude が「どう実現するか」を考えて手を動かす、という関係で開発を進めます。
...
- **テストの作成・実行**: 「このモジュールのテストを書いて、実行して、失敗を直して」
...
勝手にコードを書き換えたり、危険なコマンドを実行したりはしないので、安心して使えます。
...
最初のうちは **Ask before edits**、「いきなり実装されると不安」というときは **Plan mode** がおすすめです。Plan mode では計画書がマークダウンで開かれ、コメントを書いて方針を修正できます。
